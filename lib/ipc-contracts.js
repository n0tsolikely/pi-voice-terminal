...
function normalizeLiveSttControlPayload(value) {
  const input = isObject(value) ? value : {}
  const sessionId = normalizeSessionId(input.sessionId)

  if (!sessionId) {
    throw new Error('Live STT session ID is missing.')
  }

  return {
    sessionId,
    language: normalizeClipboardText(input.language).trim() || 'en'
  }
}

function normalizeLiveSttAudioPayload(value) {
  const input = isObject(value) ? value : {}
  const sessionId = normalizeSessionId(input.sessionId)
  const audioBuffer = normalizeBinaryPayload(input.audioBuffer)
  ...
  if (!audioBuffer.byteLength) {
    throw new Error('Live STT audio chunk is empty.')
  }
...
function normalizeSessionId(value) {
  return normalizeClipboardText(value).trim().slice(0, MAX_SESSION_ID_LENGTH)
}
