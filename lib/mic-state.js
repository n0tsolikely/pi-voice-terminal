(function bootstrapMicState(root) {
  ...
  function getMicViewModel(state) {
    ...
    return {
      ...
      modeDescription,
      modeButtonsDisabled: phase === MIC_PHASES.RECORDING || phase === MIC_PHASES.STOPPING,
      shouldShowMeter:
  ...
  function getStatusText(state) {
    switch (state.phase) {
      case MIC_PHASES.ARMED:
        return 'Auto listening is on. Speak, pause, and it will inject text into the prompt.'
      ...
      default:
        if (state.mode === MIC_MODES.HOLD) {
          return state.liveDictationSupported
            ? 'PTT mode: hold mic to talk. Words appear as you speak.'
            : 'PTT mode: hold mic to talk.'
        }
  ...
  function getModeDescription(state) {
    if (state.mode === MIC_MODES.HOLD) {
      return state.liveDictationSupported
        ? 'PTT mode: hold the mic button to talk. Dictation appears live, then final transcription settles when you release.'
        : 'PTT mode: hold the mic button to record. Release to transcribe into the prompt.'
    }
    ...
    if (!state.liveDictationSupported) {
      return 'Always listening is armed. Speak, pause, and it will transcribe into the prompt.'
    }
  ...
  root.PiVoiceTerminalMicState = api
