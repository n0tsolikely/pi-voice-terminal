...
const DEFAULT_REPO_NAME = 'pi-voice-terminal'
...
class AppUpdater {
  ...
  get launchScriptPath() {
    return path.join(this.baseDir, 'launch-pi-voice-terminal.bat')
  }
  ...
  async applyUpdate() {
    ...
    return {
      ...
      launchScriptPath: path.join(this.stableRepoDir, 'launch-pi-voice-terminal.bat')
    }
  }
...
function buildUpdatePrompt(updateInfo) {
  ...
  if (updateInfo.strategy === 'version') {
    return [
      `A newer Pi Voice Terminal build is available (${updateInfo.currentLabel} -> ${updateInfo.latestLabel}).`,
      updateInfo.migratesToStablePath
        ? 'Updating now will move this install into your standard updateable repo under USERPROFILE\\pi-voice-terminal.'
  ...
  return `A newer Pi Voice Terminal build is available (${updateInfo.currentLabel} -> ${updateInfo.latestLabel}). Update now and restart into the latest version?${dirtyNote}`
}
