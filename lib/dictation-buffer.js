(function bootstrapDictationBuffer(root) {
  const devDictionaryApi =
    typeof module !== 'undefined' && module.exports
      ? require('./dev-dictionary')
      : root.PiVoiceTerminalDevDictionary || {}
  const { applyDevDictionary = (text) => String(text || '') } = devDictionaryApi
  ...
  function normalizeDictationText(text) {
    ...
    const corrected = applyDevDictionary(normalized)
    const withoutTrailingSentencePunctuation = corrected
      .replace(/([a-z0-9\]\)\}])(?:\s*[.!?]+)+$/i, '$1')
      .trim()

    return withoutTrailingSentencePunctuation.toLowerCase()
  }
  ...
  root.PiVoiceTerminalDictationBuffer = api
