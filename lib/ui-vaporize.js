(function attachUiVaporize(globalScope) {
  ...
  function vaporizeElement(element, options = {}) {
    ...
    const snapshotProfile = createSnapshotProfile(element)
    ...
    return renderFallbackSnapshot(snapshotProfile, rect)
      .then((canvas) => ({
        canvas,
      ...
      .then((canvas) => {
        ...
        let particleState = createParticleState(canvas.canvas, rect, particleOptions)
        let renderMethod = canvas.method
        ...
        return animateParticles(overlayCanvas, particleState, particleOptions).then(() => ({
          ok: true,
          method: renderMethod,
          particleCount: particleState.length
        }))
      })
      .catch((error) => ({
        ok: false,
  ...
  function vaporizeImageDataUrl(imageDataUrl, rect, options = {}) {
    const normalizedRect = normalizeSnapshotRect(rect)
    if (!imageDataUrl || !normalizedRect) {
      return Promise.resolve({
        ok: false,
        method: 'invalid-image',
        particleCount: 0
      })
    }

    const particleOptions = {
      durationMs: clampNumber(options.durationMs, 760, 320, 920),
      particleSize: clampNumber(options.particleSize, getAdaptiveParticleSize(normalizedRect), 2, 8),
      travel: clampNumber(options.travel, Math.max(normalizedRect.width, normalizedRect.height) * 0.12, 20, 64),
      gravity: clampNumber(options.gravity, Math.max(normalizedRect.height * 0.08, 10), 6, 30),
      delayMs: clampNumber(options.delayMs, 0, 0, 240)
    }

    return loadImageSnapshotCanvas(imageDataUrl, normalizedRect)
      .then((snapshotCanvas) => {
        const overlayCanvas = buildOverlayCanvas(normalizedRect, particleOptions)
        const particleState = createParticleState(snapshotCanvas, normalizedRect, particleOptions)

        if (!particleState.length) {
          overlayCanvas.remove()
          return {
            ok: false,
            method: 'image-snapshot',
            particleCount: 0
          }
        }
  ...
  function createSnapshotProfile(element) {
    ...
    return {
      clone,
      text: collectTextContent(element),
      style: {
        backgroundColor: computedStyle.backgroundColor || 'rgba(12, 12, 12, 0.88)',
        borderColor: computedStyle.borderColor || 'rgba(255, 255, 255, 0.14)',
        borderWidth: parseFloat(computedStyle.borderWidth) || 1,
        borderRadius: parseFloat(computedStyle.borderRadius) || 16,
        color: computedStyle.color || '#f3f3f3',
        font: `${computedStyle.fontWeight || '500'} ${computedStyle.fontSize || '12px'} ${computedStyle.fontFamily || 'sans-serif'}`
      }
    }
  }
  ...
  function renderFallbackSnapshot(snapshotProfile, rect) {
    const pixelRatio = Math.max(1, Math.min(window.devicePixelRatio || 1, 2))
    const width = Math.max(1, Math.ceil(rect.width))
    const height = Math.max(1, Math.ceil(rect.height))
    const canvas = document.createElement('canvas')
    const context = canvas.getContext('2d')
    const bubbleStyle = snapshotProfile.style
    const radius = bubbleStyle.borderRadius || 16
    ...
    context.fillStyle = bubbleStyle.backgroundColor
    context.fill()
    context.strokeStyle = bubbleStyle.borderColor
    context.lineWidth = bubbleStyle.borderWidth || 1
    context.stroke()

    context.fillStyle = bubbleStyle.color
    context.font = bubbleStyle.font
    context.textBaseline = 'top'
    drawWrappedText(context, snapshotProfile.text, 14, 12, width - 28, 16)

    return Promise.resolve(canvas)
  }

  function loadImageSnapshotCanvas(imageDataUrl, rect) {
    return new Promise((resolve, reject) => {
      ...
      image.onerror = () => {
        reject(new Error('Failed to load close vaporize snapshot image.'))
      }
  ...
  function normalizeSnapshotRect(rect) {
  ...
  globalScope.PiVoiceTerminalVaporize = {
    vaporizeElement,
    vaporizeImageDataUrl
  }
})(window)
